      ...
      context 'with cards intro text where the intro send fails and the cards send would succeed' do
        it 'stops before sending the cards template and stays retryable' do
          allow(HTTParty).to receive(:post).and_return(
            ...
            instance_double(HTTParty::Response, success?: true, body: { message_id: 'mid_intro' }.to_json,
                                                parsed_response: { 'message_id' => 'mid_intro' }),
            instance_double(HTTParty::Response, success?: true, body: { message_id: 'mid_cards' }.to_json,
                                                parsed_response: { 'message_id' => 'mid_cards' })
          )
          ...
          expect(HTTParty).to have_received(:post).exactly(3).times
          expect(message.reload.source_id).to eq('mid_cards')
        end
      end
